...
with cols2:
    ...
    with container:
        if st.session_state['radio2']=='Input Data':
            chatbot_input_data(disable=False)
        elif st.session_state['radio2']=="Model Output":
            chatbot_model_output(disable=False)
        else:
            chatbot1('Model Build and Performance',False) 
...
with cols1:
    ...
    def plot_response_curves():
        ...
        fig.update_yaxes(title='Prospects')
        return fig
    ...
    if auth_status == True:
        ...
        #     chatbot("Model Build and Performance")
        ...
        with sub_header[1]:
            st.metric(label = 'Prospects', value=format_numbers(_scenario.actual_total_sales,include_indicator=False))
        ...
        with sub_header[3]:
            st.metric(label = 'Prospects', 
                        value=format_numbers((_scenario.actual_total_sales),include_indicator=False), 
        ...
        with st.expander("Channel Spends Simulator"):
            ...
            with _columns[3]:
                generate_spending_header('Prospects')
            with _columns[4]:
            ...
            for i,channel_name in enumerate(channels_list):
                ...
                with _columns[1]:
                    ...
                    channel_spends = (_channel_class.actual_total_spends )
                    min_value = ((1+channel_bounds[0]/100) * channel_spends )
                    max_value = ((1+channel_bounds[1]/100) * channel_spends )
                ...
                with _columns[2]:
                    # spends
                    current_channel_spends = (_channel_class.modified_total_spends *  _channel_class.conversion_rate)
                    actual_channel_spends = (_channel_class.actual_total_spends * _channel_class.conversion_rate)
                    spends_delta = (_channel_class.delta_spends * _channel_class.conversion_rate)
                    ## REMOVE 
                    st.metric('Spends',
                ...
                with _columns[3]:
                    # sales
                    current_channel_sales = (_channel_class.modified_total_sales)
                    actual_channel_sales = (_channel_class.actual_total_sales)
                    sales_delta = (_channel_class.delta_sales)
                    st.metric('Prospects',
